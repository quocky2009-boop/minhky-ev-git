-- ============================================================
-- Migration 122: Bổ sung nốt phần CÒN THIẾU sau khi rà soát —
-- 114 (fix Chi NCC ghi nhầm Thu) và 115a (Hủy đơn đồng bộ
-- paid_amount, Dịch vụ/Đặt cọc nhận account_id) ĐỀU CHƯA TỪNG CHẠY.
-- Chạy SAU migration 121. Idempotent, chạy lại nhiều lần vẫn an toàn.
-- ============================================================
...
-- ---------- Bo sung phan con thieu cua 114: fn_tra_no_ncc dung dung _auto_chi ----------
...
-- SUA DU LIEU LICH SU: cac ban ghi da bi ghi NHAM thanh Thu (loi phan mem, sua truc tiep)
update public.cash_txns
set direction = 'Chi',
    description = concat(description, ' · [ĐÃ SỬA: bản ghi này trước đó bị hệ thống ghi NHẦM thành Thu do lỗi phần mềm — migration 122 tự động sửa lại thành Chi cho đúng bản chất]')
where category = 'Chi trả NCC' and direction = 'Thu';

do $$
begin
  raise notice '✅ Migration 122 hoàn tất — fn_xoa_don/fn_tra_hang_ban/fn_dv_thu_tien/fn_dat_coc/fn_tra_no_ncc đều đã cập nhật bản mới nhất.';
end $$;
