-- ============================================================
-- Migration 097: FIX BUG — fn_sua_don_ban hoàn toàn bỏ qua tham số
-- "new_payments" khi Sửa đơn bán có "Thu thêm" — không tạo dòng
-- sale_payments mới, không ghi vào sổ quỹ, không cập nhật paid_amount.
-- Giờ xử lý đúng: Tiền mặt/Chuyển khoản ghi thu ngay vào quỹ; Trả góp
-- vào trạng thái "Chờ giải ngân" (đồng bộ với fix migration 096).
-- Chạy sau 096. Chạy lại nhiều lần vẫn an toàn.
-- ============================================================
...
declare me record; o record; u record; it jsonb; pm jsonb; fe jsonb;
  ...
  v_sp_id bigint; v_cash_code text; v_them bigint := 0;
begin
  ...
  -- ===== FIX: xu ly THU THEM (new_payments) — truoc day bi bo qua hoan toan =====
  for pm in select jsonb_array_elements(coalesce(p->'new_payments','[]'::jsonb)) loop
    ...
    v_them := v_them + (pm->>'amount')::bigint;
  ...
  if v_them > 0 then
    update public.sales_orders set paid_amount = coalesce(paid_amount,0) + v_them where id = o.id;
  end if;
