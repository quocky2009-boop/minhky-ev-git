-- ============================================================
-- Migration 103: Mở rộng "Chỉ tiêu doanh số" (sales_targets) để hỗ
-- trợ đặt theo KHU VỰC (thay vì chỉ theo từng cửa hàng) + theo HÃNG
-- XE trong phạm vi đó (VD: Khu vực Hàm Yên tháng 8: 60 xe VinFast +
-- 50 xe TAILG).
--
-- Quy tắc phạm vi (chỉ 1 trong 2 được set, không cả hai):
--   location_code có giá trị  -> chỉ tiêu riêng 1 cửa hàng
--   khu_vuc có giá trị        -> chỉ tiêu cho cả khu vực (nhiều cửa hàng)
--   cả 2 đều NULL             -> chỉ tiêu toàn hệ thống
-- brand NULL trên 1 dòng = chỉ tiêu TỔNG của phạm vi đó (không tách hãng);
-- brand có giá trị = chỉ tiêu RIÊNG cho hãng đó trong phạm vi.
--
-- Đổi unique constraint cũ (thang, location_code) sang unique index
-- dùng coalesce (vì NULL không so sánh unique được theo cách thông
-- thường trong Postgres).
-- Chạy sau 102. Chạy lại nhiều lần vẫn an toàn.
-- ============================================================
...
alter table public.sales_targets drop constraint if exists sales_targets_thang_location_code_key;
drop index if exists sales_targets_thang_location_code_key;

create unique index if not exists sales_targets_uniq_idx on public.sales_targets
  (thang, coalesce(location_code,''), coalesce(khu_vuc,''), coalesce(brand,''));

create or replace function public.fn_luu_chi_tieu(p jsonb)
returns void language plpgsql security definer set search_path = public as $$
declare me record; v_thang date; v_loc text; v_kv text; v_brand text;
begin
  ...
  v_kv := nullif(p->>'khu_vuc','');
  v_brand := nullif(p->>'brand','');

  if v_loc is not null and v_kv is not null then
    raise exception 'CHON_SAI_PHAM_VI: chỉ được chọn 1 trong 2 — theo cửa hàng HOẶC theo khu vực, không cả hai';
  end if;

  insert into public.sales_targets (thang, location_code, khu_vuc, brand, target_revenue, target_units, note, created_by, created_by_name)
  values (v_thang, v_loc, v_kv, v_brand, coalesce((p->>'target_revenue')::bigint,0),
    coalesce((p->>'target_units')::int,0), coalesce(p->>'note',''), me.uid, me.name)
  on conflict (thang, coalesce(location_code,''), coalesce(khu_vuc,''), coalesce(brand,'')) do update set
    target_revenue = excluded.target_revenue, target_units = excluded.target_units,
    note = excluded.note, updated_at = now();
end $$;

create or replace function public.fn_xoa_chi_tieu(p_id bigint)
returns void language plpgsql security definer set search_path = public as $$
declare me record;
begin
  select * into me from public.fn_me();
...
-- ---------- Cập nhật fn_ai_bao_cao_dieu_hanh: chi_tieu_thang giờ ưu tiên
-- cửa hàng > khu vực > toàn hệ thống, và thêm mảng theo_hang ----------
...
declare l record; v_hom_qua date := p_ngay - 1; v_7ngay_truoc date := p_ngay - 6;
  ...
  v_ct_thang record; v_ct_theo_hang jsonb;
...
begin
  ...
  loop
    ...
    -- Chi tieu TONG (brand IS NULL): uu tien cua hang > khu vuc > toan he thong
    select * into v_ct_thang from public.sales_targets st
    where st.thang = date_trunc('month', p_ngay)::date and st.location_code = l.code and st.brand is null;
    if v_ct_thang is null then
      select * into v_ct_thang from public.sales_targets st
      where st.thang = date_trunc('month', p_ngay)::date and st.khu_vuc = l.region and st.location_code is null and st.brand is null;
    end if;
    if v_ct_thang is null then
      select * into v_ct_thang from public.sales_targets st
      where st.thang = date_trunc('month', p_ngay)::date and st.location_code is null and st.khu_vuc is null and st.brand is null;
    end if;

    -- Chi tieu THEO HANG (brand IS NOT NULL): gop ca chi tieu rieng cua hang
    -- va chi tieu chung cua ca khu vuc (neu chua co chi tieu rieng cua hang)
    select coalesce(jsonb_agg(jsonb_build_object('brand', x.brand, 'muc_tieu_so_xe', x.target_units, 'muc_tieu_doanh_thu', x.target_revenue)), '[]'::jsonb)
      into v_ct_theo_hang
    from (
      select distinct on (st.brand) st.brand, st.target_units, st.target_revenue
      from public.sales_targets st
      where st.thang = date_trunc('month', p_ngay)::date and st.brand is not null
        and (st.location_code = l.code or (st.khu_vuc = l.region and st.location_code is null))
      order by st.brand, (st.location_code is not null) desc  -- uu tien dong theo cua hang truoc dong theo khu vuc
    ) x;
    ...
    -- ===== 5) QUY (chi tinh tai khoan RIENG cua cua hang nay) =====
    ...
    return query select
      ...
      jsonb_build_object(
        ...
        'chi_tieu_theo_hang', v_ct_theo_hang
      ),
