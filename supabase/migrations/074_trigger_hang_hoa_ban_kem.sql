-- ============================================================
-- Migration 074 — TRIGGER TỰ TRỪ TỒN HÀNG HÓA KHI BÁN KÈM
--  Thay vi sua lai toan bo fn_ban_hang_v2, dung trigger AFTER INSERT
--  tren sale_items: neu co product_id -> tu dong tru ton hang hoa
--  va hoan ton khi DELETE (huy/sua don)
-- Chạy SAU 073. Chạy lại nhiều lần vẫn an toàn.
-- ============================================================

-- Function trigger tru ton khi insert sale_item co product_id
  ...
  if TG_OP = 'INSERT' then
    if NEW.product_id is not null then
      ...
      if prod is not null then
        if prod.stock_qty < NEW.qty then
          raise exception 'KHONG_DU_TON: % chỉ còn % %, cần % %',
            prod.name, prod.stock_qty, prod.unit, NEW.qty, prod.unit;
        end if;
        v_before := prod.stock_qty;
        update public.products set stock_qty = stock_qty - NEW.qty, updated_at = now()
        ...
        values (NEW.product_id, 'Xuất', NEW.qty, v_before, v_before - NEW.qty,
          NEW.unit_price, NEW.sale_code, 'Bán kèm đơn ' || NEW.sale_code,
    ...
    -- Hoan ton khi xoa sale_item (sua don / huy don)
    if OLD.product_id is not null then
      ...
      if prod is not null then
        ...
        values (OLD.product_id, 'Nhập', OLD.qty, v_before, v_before + OLD.qty,
          OLD.unit_price, OLD.sale_code, 'Hoàn tồn khi sửa/hủy đơn ' || OLD.sale_code,
          (select id from public.profiles where id = auth.uid() limit 1),
          (select name from public.profiles where id = auth.uid() limit 1));
      end if;
    end if;
...
-- Gan trigger vao sale_items
drop trigger if exists trg_sale_item_hang_hoa on public.sale_items;
create trigger trg_sale_item_hang_hoa
  after insert or delete on public.sale_items
  for each row execute function public._trg_sale_item_hang_hoa();

-- Cap nhat fn_ban_hang_v2: them product_id vao insert sale_items
-- (chi sua phan insert sale_items, giu nguyen phan con lai)
...
begin
  ...
  for it in select jsonb_array_elements(coalesce(p->'items','[]'::jsonb)) loop
    ...
    -- Trigger _trg_sale_item_hang_hoa se tu xu ly tru ton neu product_id <> null
...
do $do$
begin
  raise notice 'XONG 074: trigger tu tru ton hang hoa khi ban kem don xe; fn_ban_hang_v2 them product_id';
end $do$;
