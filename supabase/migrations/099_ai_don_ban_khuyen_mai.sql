-- ============================================================
-- Migration 099: Bổ sung field "chuong_trinh_khuyen_mai" vào
-- fn_ai_don_ban (tool AI get_sales_orders_list) — khuyến mại nằm ở
-- bảng riêng (sale_order_promotions) nên không tự có trong
-- to_jsonb(o), cần thêm bằng subquery.
...
language sql security definer stable set search_path = public as $$
  select
    ...
    || jsonb_build_object(
        ...
        'chuong_trinh_khuyen_mai', coalesce((
          select jsonb_agg(pr.name) from public.sale_order_promotions sop
          join public.promotions pr on pr.id = sop.promotion_id
          where sop.sale_code = o.code), '[]'::jsonb)
      )
