...
const empty = { id: null, customer_id: "", interested_vehicle_id: "", test_drive_vehicle_id: "",
  ...
  customer_feedback: "", employee_note: "" };
...
export default function LaiThu() {
  ...
  const [tab, setTab] = useState("cua_toi");
  const [choDuyet, setChoDuyet] = useState([]);

  const canDuyet = profile && ["CEO", "MANAGER", "ADMIN"].includes(profile.role);

  const load = async () => {
    if (!profile) return;
    const [{ data: c }, { data: r }, { data: cd }] = await Promise.all([
      ...
      supabase.from("test_drives").select("*").in("status", ["submitted", "needs_revision"]).order("submitted_at").limit(300),
    ]);
    setCusts(c || []); setRows(r || []); setChoDuyet(cd || []);
  };
  ...
  const duyet = async (o, action) => {
    let note = "";
    if (["revise", "reject"].includes(action)) {
      note = prompt(action === "revise" ? "Lý do yêu cầu bổ sung:" : "Lý do từ chối:");
      if (note === null) return;
      if (!note.trim()) return notify("Bắt buộc nhập lý do.", "err");
    }
    const { error } = await supabase.rpc("fn_mkt_duyet_lai_thu", { p_id: o.id, p_action: action, p_note: note });
    if (error) return notify(errMsg(error), "err");
    notify(action === "approve" ? "Đã xác nhận lượt lái thử." : action === "revise" ? "Đã yêu cầu bổ sung." : "Đã từ chối.");
    load();
  };
  ...
  const openEdit = (o) => {
    setF({ id: o.id, customer_id: o.customer_id, interested_vehicle_id: o.interested_vehicle_id || "",
      ...
      customer_feedback: o.customer_feedback || "", employee_note: o.employee_note || "", });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="font-extrabold text-lg mr-auto">Khách lái thử</div>
      ...
      {canDuyet && (
        <div className="flex gap-1.5">
          <button className={`btn !px-3 !py-2 !text-xs ${tab === "cua_toi" ? "bg-brand text-white" : "bg-[#EEF1F4]"}`} onClick={() => setTab("cua_toi")}>Của tôi ({rows.length})</button>
          <button className={`btn !px-3 !py-2 !text-xs ${tab === "duyet" ? "bg-brand text-white" : "bg-[#EEF1F4]"}`} onClick={() => setTab("duyet")}>
            Chờ duyệt {choDuyet.filter((x) => x.status === "submitted").length > 0 && <span className="ml-1 px-1.5 rounded-full bg-danger text-white">{choDuyet.filter((x) => x.status === "submitted").length}</span>}
          </button>
        </div>
      )}
      ...
      {tab === "duyet" && canDuyet ? (
        <div className="card">
          <div className="font-extrabold mb-2">Lượt lái thử chờ xác nhận ({choDuyet.length})</div>
          {choDuyet.length === 0 ? <div className="text-sm text-[#8A93A0]">Không có lượt nào chờ duyệt.</div> : (
                ...
                <div key={o.id} className={`p-3 rounded-xl border ${o.is_suspicious ? "border-[#F0C000] bg-[#FFFCF0]" : "border-[#E3E8EF]"}`}>
                  <div className="flex items-center gap-2 flex-wrap">
                    <div className="mr-auto min-w-0">
                      ...
                      <div className="text-[11px] text-[#8A93A0]">{o.code} · {o.employee_name} · lái thử {vName(o.test_drive_vehicle_id)} · {NEED_LABEL[o.customer_need_level]} · {RESULT_LABEL[o.result_status]}</div>
                  ...
                  <div className="flex gap-1.5 mt-2.5 flex-wrap">
                    <button className="btn-ok !px-3 !py-1.5 !text-xs" onClick={() => duyet(o, "approve")}>✓ Xác nhận</button>
                    <button className="btn-ghost !px-3 !py-1.5 !text-xs" onClick={() => duyet(o, "revise")}>Yêu cầu bổ sung</button>
                    <button className="btn-ghost !px-3 !py-1.5 !text-xs !text-danger" onClick={() => duyet(o, "reject")}>Từ chối</button>
                  </div>
                </div>
              ))}
            </div>
