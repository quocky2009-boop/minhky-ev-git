...
// In dang HOA DON MAY IN NHIET (kho 80mm) — dung cho may in bill sieu thi
export async function printOrderBill({ supabase, o, vehicles, locations, settings, notify }) {
    const { data: its } = await supabase.from("sale_items").select("*").eq("sale_code", o.code);
    const v = vehicles.find((x) => x.id === o.vehicle_id);
    const l = locations.find((x) => x.code === o.location_code);
    const items = its || [];
    ...
    const tong = tienXe + kem;
    const paid = o.paid_amount || 0;
    const conLai = Math.max(tong - paid, 0);
    const money = (n) => new Intl.NumberFormat("vi-VN").format(n) + "đ";
    const line = (a, b, bold) => `<div class="ln${bold ? " b" : ""}"><span>${a}</span><span>${b}</span></div>`;
    const html = `<!DOCTYPE html><html lang="vi"><head><meta charset="utf-8"><title>${o.code}</title><style>
      *{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',Arial,sans-serif}
      @page{size:80mm auto;margin:0}
      body{width:80mm;padding:4mm 3mm;color:#000;font-size:12px;line-height:1.45}
      .ct{text-align:center}
      .cty{font-size:14px;font-weight:800}
      .sub{font-size:10.5px}
      .ti{font-size:15px;font-weight:800;margin:6px 0 2px}
      .dash{border-top:1px dashed #000;margin:6px 0}
      .ln{display:flex;justify-content:space-between;gap:6px;font-size:11.5px;padding:1px 0}
      .ln.b{font-weight:800;font-size:13px}
      .it{font-size:11.5px;padding:2px 0}
      .it .nm{font-weight:600}
      .it .dt{display:flex;justify-content:space-between;color:#000;font-size:11px}
      .rt{text-align:right}.ce{text-align:center}
      .chu{font-style:italic;font-size:11px;margin-top:4px}
      .ft{text-align:center;font-size:10.5px;margin-top:8px}
      .sk{font-family:monospace;font-size:11px}
      @media print{body{width:80mm}.noprint{display:none}}
    </style></head><body>
      <div class="ct">
        <div class="cty">${settings.cty_ten || "XE ĐIỆN MINH KỲ"}</div>
        ${settings.cty_diachi ? `<div class="sub">${settings.cty_diachi}</div>` : ""}
        ${settings.cty_sdt ? `<div class="sub">ĐT: ${settings.cty_sdt}</div>` : ""}
        <div class="ti">PHIẾU BÁN XE</div>
        <div class="sub">${l?.name || o.location_code}</div>
      </div>
      <div class="dash"></div>
      ${line("Số phiếu", `<b>${o.code}</b>`)}
      ${line("Ngày", new Date(o.sale_date).toLocaleDateString("vi-VN"))}
      ${line("Khách", o.customer_name)}
      ${line("SĐT", o.customer_phone)}
      <div class="dash"></div>
      <div class="it">
        <div class="nm">${v ? v.brand + " " + v.name + " " + v.color : o.vehicle_id}</div>
        <div class="sk">SK: ${o.frame_number}</div>
        <div class="dt"><span>${o.quantity} x ${money(o.sale_price)}</span><span>${money(tienXe)}</span></div>
      </div>
      ${items.map((x) => `<div class="it"><div class="nm">${x.name}</div><div class="dt"><span>${x.qty} x ${money(x.unit_price)}</span><span>${money(x.amount)}</span></div></div>`).join("")}
      <div class="dash"></div>
      ${line("TỔNG CỘNG", money(tong), true)}
      ${line("Đã thanh toán", money(paid))}
      ${line("Còn lại", money(conLai), true)}
      <div class="chu">Bằng chữ: ${docTien(tong)}</div>
      ${o.payment_method ? line("Hình thức", o.payment_method) : ""}
      ${o.note ? `<div style="font-size:11px;margin-top:4px">Ghi chú: ${o.note}</div>` : ""}
      <div class="dash"></div>
      <div class="ft">
        ${settings.phieu_footer || "Cảm ơn Quý khách đã tin tưởng Minh Kỳ EV!"}
        <div style="margin-top:6px">NV bán: ${o.seller_name}</div>
      </div>
      <div class="noprint ce" style="margin-top:12px"><button onclick="window.print()" style="padding:8px 22px;font-size:13px;font-weight:700;cursor:pointer">🖨 In bill</button></div>
    </body></html>`;
    const w = window.open("", "_blank");
    if (!w) return notify("Trình duyệt chặn cửa sổ mới — cho phép popup cho trang này rồi bấm In lại.", "err");
    w.document.write(html); w.document.close();
